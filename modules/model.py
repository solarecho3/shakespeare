...
class Data:
    ...
    This prevents making one thread of all classes in the model.
    """

    data: DataLoader.dataset  # just annotations to demonstrate the setting class
    data_configurations: DataLoader.__init__  # everything moves linearly top to bottom
    # and all data use this dataclass for global sharing
...
class DataLoader:
    ...
    def __init__(self, path: os.PathLike = "../config/", **kwargs) -> DataLoader:
        ...
        if kwargs is None:
            logging.critical(
                f"Specify a data set. Refer to {path} to configure data sets for loading."
            )
            ...
            try:
                with open(
                    Data.data_configurations[kwargs]["path"], "r", encoding="utf-8-sig"
                ) as data_file:
...
class VocabularyConfigurer:
    ...
    def __init__(self, type):
        ...
        if type == "char":
            ...
            raise KeyError(
                "VocabularyConfigurer constructor called with improper arguments."
            )
    ...
    def _create_vocabulary(self):
        ...
        if Data.vocabulary_type == "char":
            ...
            raise AttributeError(
                "Vocabulary failed to create. Check arguments for the VocabularyConfigurer constructor."
            )
...
class Encoder:
    ...
    def __init__(self, text):
        ...
        logging.info(
            f"Data encoded using {Data.vocabulary_type} tokenization: {Data.encoded_data[:50]}..."
        )
...
class Decoder:
    ...
    def __init__(self, text):
        ...
        logging.info(
            f"Data decoded using {Data.vocabulary_type} tokenization: {Data.decoded_data[:500]}..."
        )
...
class Trainer:
    ...
    def __init__(self):
        ...
        self.training_validation_pivot_point = int(
            self.training_set_percentage * len(self.encoded_data)
        )
    ...
    def train(self):
        ...
        Decoder(
            self.m.generate(context, max_new_tokens=HyperParams.max_gen_tokens)[
                0
            ].tolist()
        )
...
class Head(torch.nn.Module):
    ...
    def __init__(self, head_size):
        ...
        self.key = torch.nn.Linear(
            HyperParams.embedding_table_dims, head_size, bias=False
        )
        ...
        self.register_buffer(
            "tril",
            torch.tril(torch.ones(HyperParams.block_size, HyperParams.block_size)),
        )

        self.dropout = torch.nn.Dropout(HyperParams.dropout)
...
class MultiHeadAttention(torch.nn.Module):
    ...
    def __init__(self, num_heads, head_size):
        ...
        self.projection = torch.nn.Linear(
            HyperParams.embedding_table_dims, HyperParams.embedding_table_dims
        )
        self.dropout = torch.nn.Dropout(HyperParams.dropout)
...
class FeedForward(torch.nn.Module):
    def __init__(self, num_embeds):
        ...
        self.net = torch.nn.Sequential(
            torch.nn.Linear(num_embeds, 4 * num_embeds),
            torch.nn.ReLU(),
            torch.nn.Linear(4 * num_embeds, num_embeds),
...
class Block(torch.nn.Module):
    """
    Transformer block: communication, then computation (query, key)
    """

    def __init__(self, num_embeds, num_heads):
        # num_embeds - embedding dimension, num_heads - number of heads
        super().__init__()
        head_size = num_embeds // num_heads
        self.sa = MultiHeadAttention(num_heads, head_size)
        self.ffwd = FeedForward(num_embeds)
        self.ln1 = torch.nn.LayerNorm(num_embeds)
        self.ln2 = torch.nn.LayerNorm(num_embeds)
...
class BigramLanguageModel(torch.nn.Module):
    def __init__(self):
        ...
        self.token_embedding_table = torch.nn.Embedding(
            Data.vocabulary_size, HyperParams.embedding_table_dims
        )
        logging.info(
            f"Embedding table created: {self.token_embedding_table} with vocabulary size {Data.vocabulary_size}"
        )

        self.position_embedding_table = torch.nn.Embedding(
            HyperParams.block_size, HyperParams.embedding_table_dims
        )
        self.blocks = torch.nn.Sequential(
            *[
                Block(HyperParams.embedding_table_dims, num_heads=HyperParams.n_heads)
                for _ in range(HyperParams.n_layers)
            ]
        )
        self.ln_f = torch.nn.LayerNorm(HyperParams.embedding_table_dims)
        self.language_modeling_head = torch.nn.Linear(
            HyperParams.embedding_table_dims, Data.vocabulary_size
        )
        ...
        # self.feed_forward = FeedForward(HyperParams.embedding_table_dims)
        self.language_modeling_head = torch.nn.Linear(
            HyperParams.embedding_table_dims, Data.vocabulary_size
        )
    ...
    def forward(self, idx, targets=None):
        ...
        positional_embeddings = self.position_embedding_table(
            torch.arange(T, device=HyperParams.device)
        )  # (T,C)
